...
- [gomemlimit](#gomemlimit) — Лимит памяти Go у ядра Xray (GOMEMLIMIT)
...
Ограничивает память, которую разрешено занимать Go-рантайму внутри ядра Xray (`GOMEMLIMIT`), или снимает ограничение. Перезапускается **только процесс Xray внутри контейнера**: панель, планировщик, база и открытые сессии в дашборде не трогаются.

Зачем это нужно: ядро Xray написано на Go, а Go растит хип до срабатывания сборщика и отдаёт память системе неохотно. На панели с большим числом пользователей RSS процесса xray уезжает в гигабайты, и при нехватке памяти OOM-killer выбирает самый крупный процесс — обычно это сам xray или база. С `GOMEMLIMIT` сборщик начинает работать тем активнее, чем ближе процесс к границе, и держится в заданном объёме.

**Посмотреть текущее состояние (подкоманда по умолчанию):**
...
```
Xray memory limit (GOMEMLIMIT)
  configured: 1GiB  (/var/lib/marzban/xray_go_env)
  running:    pid 1234, limit 1GiB, RSS 412 MiB
```

**Выставить лимит:**

```bash
marzban gomemlimit set 1G
marzban gomemlimit set 800MiB
marzban gomemlimit set 1073741824
```

**Снять лимит:**

```bash
marzban gomemlimit off
```

**Разово с GitHub, без установленного скрипта:**

```bash
sudo bash -c "$(curl -sL https://raw.githubusercontent.com/V2as/Sauceban/master/sauceme.sh)" @ gomemlimit set 1G
```

| Подкоманда | Описание |
|---|---|
| `show` (по умолчанию) | Что записано в файле, с каким лимитом реально работает процесс xray и сколько памяти он занимает |
| `set <размер>` | Выставить лимит и дождаться, пока ядро на нём заработает |
| `off` (`unset`, `remove`) | Убрать лимит совсем |

| Опция | Описание |
|---|---|
| `--force` | Разрешить лимит меньше 64MiB |
| `-h`, `--help` | Справка по команде |

**Размеры двоичные:** `1G` = `1GiB` = 1073741824 байта, `512M` = `512MiB`. Значение приводится к канонической форме (`1GiB`), потому что сам Go-рантайм принимает только суффиксы `B`/`KiB`/`MiB`/`GiB`/`TiB` — на `1G` или `1GB` он падает при старте. Писать можно в любом виде, команда пересчитает.

**Что происходит при `set` / `off`:**

1. Значение пишется в `/var/lib/marzban/xray_go_env` одной строкой `GOMEMLIMIT=...` (`off` — файл удаляется)
2. Панель читает этот файл каждый раз, когда запускает ядро, а её health-check (`JOB_CORE_HEALTH_CHECK_INTERVAL`, по умолчанию 10 с) перезапускает Xray, как только файл разошёлся с запущенным процессом
3. Команда ждёт, пока нужное значение не появится в окружении живого процесса xray (`/proc/<pid>/environ` внутри контейнера), и ещё несколько секунд проверяет, что это тот же процесс, а не перезапуск по кругу
4. Если применить не удалось — файл возвращается к прежнему значению, команда дожидается возврата ядра и завершается с кодом 1. Тихого «вроде выполнилось» не бывает: либо лимит подтверждён на живом процессе, либо ошибка

**Ограничения:**

- Лимит **мягкий**. Go выйдет за него, если столько памяти действительно занято живыми объектами — иначе рантайму осталось бы только падать. Жёсткая граница — это `mem_limit` контейнера, но она убивает процесс, а не заставляет его подчищать за собой.
- Меньше 64MiB без `--force` нельзя: на таком лимите сборщик жжёт CPU вместо освобождения памяти.
- **Удалённые ноды не затрагиваются** — там свой процесс Xray под управлением marzban-node, до него эта команда не достаёт.
- Нужен запущенный контейнер панели (`marzban up`). Если панель старее самой фичи, команда сразу скажет, что `XRAY_GO_ENV_FILE` в её конфиге нет, и посоветует `marzban update`.
- Файл лежит в `/var/lib/marzban`, поэтому лимит переживает `restart`, `down`/`up` и `update`.

Значение видно и через API панели: `GET /api/core` отдаёт поле `memory_limit` — это `GOMEMLIMIT` работающего ядра (`null`, если лимита нет).
...
|---|---|---|
| `home.html` | Главная страница (вариант 1) — маскировка под интернет-магазин | `/var/lib/marzban/templates/home/index.html` |
| `home2.html` | Главная страница (вариант 2) — футуристический редирект на cheapchat.net | `/var/lib/marzban/templates/home/index.html` |
| `sub.html` | Страница подписки (`/sub/<token>`) — редирект в `blacktemple://import/` | `/var/lib/marzban/templates/subscription/index.html` |

**Обновить оба шаблона (по умолчанию, вариант 1 главной страницы):**

```bash
sudo bash -c "$(curl -sL https://raw.githubusercontent.com/V2as/Sauceban/master/sauceme.sh)" @ update-html
```

**Обновить только главную страницу (вариант 1 — GloMart):**

```bash
sudo bash -c "$(curl -sL https://raw.githubusercontent.com/V2as/Sauceban/master/sauceme.sh)" @ update-html --home
```

**Обновить только главную страницу (вариант 2 — редирект на cheapchat.net):**

```bash
sudo bash -c "$(curl -sL https://raw.githubusercontent.com/V2as/Sauceban/master/sauceme.sh)" @ update-html --home --home-variant 2
```

**Обновить все шаблоны с вариантом 2 главной страницы:**

```bash
sudo bash -c "$(curl -sL https://raw.githubusercontent.com/V2as/Sauceban/master/sauceme.sh)" @ update-html --home-variant 2
```
...
| Опция | Описание |
|---|---|
| `--home` | Обновить только шаблон главной страницы |
| `--home-variant <1\|2>` | Выбор варианта главной страницы (по умолчанию: `1`) |
| `--sub` | Обновить только шаблон подписки |
| `--all` | Обновить все шаблоны (по умолчанию) |
| `--status` | Показать текущий статус шаблонов |

**Варианты главной страницы:**

| Вариант | Файл | Описание |
|---|---|---|
| `1` (по умолчанию) | `home.html` | Маскировка под интернет-магазин «GloMart». При попытке нажать на товар показывается форма регистрации, при нажатии «Зарегистрироваться» выводится сообщение «Доступ запрещён по вашему IP» |
| `2` | `home2.html` | Футуристическая страница с надписью «Перенаправляю вас с API на главную сайта» и автоматическим редиректом на https://cheapchat.net через 4 секунды |

**Как работают шаблоны:**

- **home.html (вариант 1)** — при заходе на корневой URL панели (без `/dashboard`) посетитель видит страницу интернет-магазина «GloMart». При попытке нажать на товар показывается форма регистрации, при нажатии «Зарегистрироваться» выводится сообщение «Доступ запрещён по вашему IP».
- **home2.html (вариант 2)** — при заходе на корневой URL панели посетитель видит футуристическую анимированную страницу с прогресс-баром и надписью «Перенаправляю вас с API на главную сайта», после чего через 4 секунды происходит автоматический редирект на https://cheapchat.net.
...
Исправить volume `acme.sh` в `docker-compose.yml`. На ранних версиях dd.sh в docker-compose монтировалась конкретная поддиректория сертификатов (`/root/.acme.sh/<домен>_ecc`), что ломалось при смене домена или обновлении сертификатов. Эта команда заменяет старый volume на монтирование всей директории `/root/.acme.sh:/root/.acme.sh` и перезапускает Marzban.

**Запуск:**

```bash
sudo bash -c "$(curl -sL https://raw.githubusercontent.com/V2as/Sauceban/master/sauceme.sh)" @ fix-acme
```
...
1. Находит в `docker-compose.yml` старые volume вида `/root/.acme.sh/<поддиректория>:/root/.acme.sh/<поддиректория>` и удаляет их
2. Добавляет `/root/.acme.sh:/root/.acme.sh` (если ещё не добавлен)
...
Готовит `docker-compose.yml` к работе лимитов канала («Лимиты канала» в меню панели). Лимиты ставятся правилами `tc` и `nft` на интерфейсе хоста, а контейнер не может их трогать без capability `NET_ADMIN`. Пока её нет, панель показывает:

```
Лимит не применяется
nft is not allowed to change the ruleset: the container needs NET_ADMIN ...
```
...
```bash
sudo bash -c "$(curl -sL https://raw.githubusercontent.com/V2as/Sauceban/master/sauceme.sh)" @ fix-limiter
```

| Опция | Описание |
|---|---|
| `--no-restart` | Только дописать `cap_add` в `docker-compose.yml`, контейнер не трогать |

**Что делает команда:**

1. Добавляет `cap_add: [NET_ADMIN]` сервису `marzban`, если его там нет
2. Предупреждает, если сервис не в `network_mode: host` — в bridge-сети лимиты не увидят клиентский трафик
3. Смотрит capability у *работающего* контейнера и пересоздаёт только его (база продолжает работать)
...
| `/var/lib/marzban/xray_go_env` | Лимит памяти ядра Xray (`gomemlimit`), одна строка `GOMEMLIMIT=` |
