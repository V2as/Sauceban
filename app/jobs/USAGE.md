...
| Файл | Задача |
|---|---|
| `0_xray_core.py` | старт/надзор процесса Xray, сверка его лимита памяти |
...
`core_health_check` кроме «ядро упало» сверяет и `GOMEMLIMIT`: файл
`XRAY_GO_ENV_FILE` против значения, с которым запущен процесс
(`xray.core.memory_limit`). Разошлись — перезапуск ядра, потому что Go читает
переменную только при старте процесса. Это единственный способ поменять лимит
без пересоздания контейнера панели; оператору его показывает
`marzban gomemlimit` (`DOCS.md`).
